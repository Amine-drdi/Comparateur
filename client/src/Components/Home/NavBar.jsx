...
import { FaHome, FaChevronDown, FaBloggerB, FaHeartbeat, FaCar, FaUserCircle } from "react-icons/fa";
...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    {
      name: <span className="text-blue-600 hover:underline">Mutuelle santé</span>,
      icon: <FaHeartbeat className="mr-1" />,
      content: (
        <div className="p-3">
          <a href="/compare" className="block text-center border p-3 rounded hover:bg-gray-50 max-w-[180px] mx-auto">
            <img src="/images/icons/assurance-medicale.png" alt="Mutuelle santé" className="w-20 h-20 mx-auto mb-2" />
            <p className="font-medium">Mutuelle santé</p>
            <p className="text-sm text-blue-600">Obtenir un devis</p>
          </a>
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="p-3">
          <a href="#" className="block text-center border p-3 rounded hover:bg-gray-50 max-w-[180px] mx-auto">
            <img src="/images/icons/voiture.png" alt="Auto" className="w-20 h-20 mx-auto mb-2" />
    ...
    },
    {
      ...
      content: (
        <div className="p-3">
          <a href="#" className="block text-center border p-3 rounded hover:bg-gray-50 max-w-[180px] mx-auto">
            <img src="/images/icons/maison.png" alt="Habitation" className="w-20 h-20 mx-auto mb-2" />
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <a href="/" className="flex items-center text-xl font-black">
            <span className="text-3xl text-blue-500 mr-2">
              <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2L4 5v6.09c0 5.05 3.41 9.76 8 10.91c4.59-1.15 8-5.86 8-10.91V5l-8-3zm0 15c-2.76 0-5-2.24-5-5s2.24-5 5-5s5 2.24 5 5s-2.24 5-5 5z" /></svg>
            </span>
            MonCompare
          </a>
              ...
              {menuItems.map((item, index) => (
                ...
                >
                  <button className="inline-flex items-center gap-1 text-gray-700 hover:text-blue-600 transition">
                    {item.icon}
                    <span>{item.name}</span>
                    <FaChevronDown className={`text-xs ${activeDropdown === index ? "rotate-180" : ""}`} />
                  </button>
                  <AnimatePresence>
            ...
            {menuItems.map((item, index) => (
              <div key={index}>
                ...
                >
                  ...
                  <FaChevronDown className={`ml-auto text-xs ${activeDropdown === index ? "rotate-180" : ""}`} />
                </button>
                {activeDropdown === index && (
