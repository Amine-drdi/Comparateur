...
function ComparisonForm() {
    ...
    const nextStep = () => {
        ...
        if (step === 4 && (!formData.niveauRemboursement || !formData.accepteAppel || !formData.conditionsAcceptees)) {
                ...
                <form onSubmit={(e) => { e.preventDefault(); nextStep(); }} className="space-y-6">
                    <div className="bg-sky-50 p-6 rounded-lg">
                        ...
                        <p className="text-gray-600 mb-6">
                            Par l'équilibre, vous pourrez profiter d'une couverture adaptée à vos besoins.
                        </p>

                        <div className="mb-8">
                            <h3 className="font-semibold text-gray-700 mb-4">Votre couverture santé</h3>
                            <div className="grid grid-cols-3 gap-4">
                                {[
                                    { value: 'minimale', label: 'Minimale', desc: 'Couverture essentielle' },
                                    { value: 'equilibree', label: 'Equilibrée', desc: 'Bon rapport qualité-prix' },
                                    { value: 'maximale', label: 'Maximale', desc: 'Protection complète' }
                                ].map(option => (
                                    <button
                                        key={option.value}
                                        type="button"
                                        onClick={() => setFormData({...formData, niveauRemboursement: option.value})}
                                        className={`p-4 border rounded-lg text-center ${formData.niveauRemboursement === option.value ? 'border-sky-500 bg-sky-50' : 'border-gray-300'}`}
                                    >
                                        <div className="font-semibold text-sky-800 mb-2">{option.label}</div>
                    ...
                    <div className="flex justify-between">
                        ...
                        <button
                            type="submit"
                            className="bg-sky-800 hover:bg-sky-700 text-white font-semibold py-2 px-6 rounded-lg"
                            disabled={!formData.niveauRemboursement || !formData.accepteAppel || !formData.conditionsAcceptees}
                        >
                            Continuer
                        </button>
                    </div>
